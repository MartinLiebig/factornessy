...
def __get_path__(region):
    if region == "US":
        ...
        return "ressources/Europe_5_Factors_Daily.CSV"
    elif region == "EM":
        return "ressources/EM_5_Factors.csv"
...
def __get_ff_factors__(region) -> pd.DataFrame:
    ...
    if region != "EM":
        ...
        factors = pd.read_csv(__get_path__(region), skiprows=range(0, 6))
        factors= factors.rename(columns={'Mkt-RF': 'MKT', "Unnamed: 0": "date"})
...
def estimate(index_code, start_date, region="Developed"):
    ...
    if(region!="EM"):
        ...
        data_handler = IndexDataHandler(start_date=start_date, normalize=True, frequency='END_OF_MONTH')
        data = data_handler.get_historic_stock_data(index_code)
...
if __name__ == "__main__":
    x,r, data=estimate("705382", start_date="19981101", region="Europe")
    print(r.summary())
